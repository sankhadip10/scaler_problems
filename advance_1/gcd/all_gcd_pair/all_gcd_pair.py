...
class Solution:
    ...
    def solve(self,A):
        ...
        A.sort(reverse = True)
        for i in range(n):
            #first we need to check if it ther in hm or not
            if hm.get(A[i]) is not None and hm[A[i]]>0:
                hm[A[i]] -= 1
            else:
                for j in range(len(res)):
                    gcd_ = self.gcd(res[j],A[i])

                    if hm.get((gcd_)) is None:
                        hm[gcd_] = 2
                    else:
                        hm[gcd_] = hm[gcd_] + 2
                res.append(A[i])
